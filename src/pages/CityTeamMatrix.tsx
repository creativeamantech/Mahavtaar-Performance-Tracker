...
export default function CityTeamMatrix() {
  ...
  const { execs, cities, matrix } = useMemo(() => buildCrossTab(finalRecords, targets), [finalRecords, targets]);
  ...
  return (
    <AppLayout>
      <div className="mb-4 flex items-center justify-between">
        <div>
          <h1 className="font-sans text-xl font-extrabold uppercase tracking-wide">City × Executive Metrics</h1>
          <p className="font-sans text-xs text-muted-foreground">Team Paid % · Multi-Level Filters</p>
        </div>
        <div className="flex items-center gap-3">
          <button onClick={() => exportMatrix({ execs, cities, matrix }, selectedBucket)} className="flex h-[34px] items-center gap-2 rounded-md bg-primary px-4 font-sans text-xs font-bold text-primary-foreground hover:opacity-90">
      ...
      <div className="mb-6 grid grid-cols-1 gap-4 sm:grid-cols-5 bg-card border rounded-xl shadow-sm p-4 rounded-xl">
        <div>
          ...
          <Select value={selectedState} onValueChange={setSelectedState}>
            ...
            <SelectContent className="bg-card border-[rgba(255,255,255,0.08)] text-foreground">{availableStates.map(v => <SelectItem key={v} value={v} className="hover:bg-primary/20 hover:text-primary">{v}</SelectItem>)}</SelectContent>
          </Select>
        </div>
        <div>
          ...
          <Select value={selectedCity} onValueChange={setSelectedCity}>
            ...
            <SelectContent className="bg-card border-[rgba(255,255,255,0.08)] text-foreground">{availableCities.map(v => <SelectItem key={v} value={v} className="hover:bg-primary/20 hover:text-primary">{v}</SelectItem>)}</SelectContent>
          </Select>
        </div>
        <div>
          ...
          <Select value={selectedBucket} onValueChange={setSelectedBucket}>
            ...
            <SelectContent className="bg-card border-[rgba(255,255,255,0.08)] text-foreground">{availableBuckets.map(v => <SelectItem key={v} value={v} className="hover:bg-primary/20 hover:text-primary">{v === 'All' ? 'All' : `Bucket ${v}`}</SelectItem>)}</SelectContent>
          </Select>
        </div>
        <div>
          <label className="mb-2 block font-sans text-xs font-bold text-muted-foreground uppercase tracking-[0.15em]">Executive</label>
          <Select value={selectedExec} onValueChange={setSelectedExec}>
            <SelectTrigger className="h-10 bg-background border rounded-md px-3 py-1 text-sm focus:border-accent focus:ring-1 focus:ring-accent px-3 font-sans text-xs border-[rgba(255,255,255,0.08)] bg-transparent"><SelectValue placeholder="All Execs" /></SelectTrigger>
            <SelectContent className="bg-card border-[rgba(255,255,255,0.08)] text-foreground">{availableExecs.map(v => <SelectItem key={v} value={v} className="hover:bg-primary/20 hover:text-primary">{v}</SelectItem>)}</SelectContent>
          </Select>
        </div>
        <div>
          <label className="mb-2 block font-sans text-xs font-bold text-muted-foreground uppercase tracking-[0.15em]">Alloc Date</label>
          <Select value={selectedAllocDate} onValueChange={setSelectedAllocDate}>
            <SelectTrigger className="h-10 bg-background border rounded-md px-3 py-1 text-sm focus:border-accent focus:ring-1 focus:ring-accent px-3 font-sans text-xs border-[rgba(255,255,255,0.08)] bg-transparent"><SelectValue placeholder="All Dates" /></SelectTrigger>
            <SelectContent className="bg-card border-[rgba(255,255,255,0.08)] text-foreground">{availableAllocDates.map(v => <SelectItem key={v} value={v} className="hover:bg-primary/20 hover:text-primary">{v}</SelectItem>)}</SelectContent>
          </Select>
        </div>
      </div>
          ...
          <table className="w-full font-sans text-xs min-w-[1000px]">
            <thead>
              <tr className="border-b border-border bg-muted/50">
                <th className="sticky left-0 z-10 bg-card px-4 py-3 text-center font-medium text-muted-foreground uppercase tracking-wider text-xs w-12">#</th><th className="sticky left-12 z-10 bg-card px-4 py-3 text-left font-medium text-muted-foreground uppercase tracking-wider text-xs">City</th>
                {execs.map(exec => (
                  <th key={exec} className="px-4 py-3 text-right font-medium text-muted-foreground uppercase tracking-wider text-xs whitespace-nowrap">{exec}</th>
                ))}
                <th className="px-4 py-3 text-right font-bold text-foreground uppercase tracking-wider text-xs">Grand Total %</th>
                <th className="px-4 py-3 text-right font-bold text-muted-foreground uppercase tracking-wider text-xs">Needed POS</th>
              </tr>
            </thead>
            <tbody>
              {cities.map((city, idx) => {
                ...
                return (
                  <tr key={city} className="border-b border-border transition-colors hover:bg-muted">
                    <td className="sticky left-0 z-10 bg-card/90 backdrop-blur-sm px-4 py-3 text-center font-bold text-muted-foreground border-r border-border">{idx + 1}</td><td className="sticky left-12 z-10 bg-card/90 backdrop-blur-sm px-4 py-3 font-medium whitespace-nowrap border-r border-border">{city}</td>
                    {execs.map(exec => {
                      const cell = matrix[city]?.[exec];
                    ...
                    <td className="px-4 py-3 text-right text-destructive font-bold">{cityNeededPOS <= 0 ? <span className="text-xs text-success uppercase tracking-wider">Achieved</span> : fmtCur(cityNeededPOS)}</td>
                  </tr>
                );
              })}
              {/* Grand Total Row */}
              <tr className="bg-primary/20 font-bold border-t-2 border-border">
                <td className="sticky left-0 z-10 bg-surface-1 px-4 py-4 font-sans uppercase text-primary tracking-widest text-[11px] border-r border-border">Grand Total</td>
                {execs.map(exec => {
      ...
      <div className="mt-6 bg-card border rounded-xl shadow-sm p-5 rounded-xl">
        <h3 className="mb-2 font-sans text-sm font-bold">Needed POS Details</h3>
